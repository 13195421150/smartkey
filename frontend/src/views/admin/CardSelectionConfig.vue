<template>
  <div class="space-y-4">
    ...
    <CardPoolDiagnostics :refresh-key="poolRefresh" />
    ...
    <div class="card">
      <div class="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div>
          ...
          <p class="text-sm text-muted mt-1">
            ...
            <el-tag type="warning" size="small" effect="plain" class="ml-2">按卡台实际规则选卡</el-tag>
          </p>
        </div>
      ...
      <div class="mt-3 text-xs text-subtle">
        默认列表只是一组产品配置，不代表所有已有卡；可把需要复用的卡产品加入优先级，再保存同步。
      </div>
    </div>
    ...
    <div class="card">
      <div class="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div>
          ...
          <p class="text-sm text-muted mt-1">
            选卡优先级保存后会同步到卡台 GPT 规则（<code class="mono text-xs">select_priority</code> + <code class="mono text-xs">strict_select</code>）。
      ...
      <div class="form-group">
        <label class="text-sm">优先产品码（启用本站策略时生效，留空使用上方优先级）</label>
        <el-input v-model="policy.product_code" placeholder="例如 P5378OX" clearable />
      </div>
      <div class="flex flex-wrap items-center gap-4 mt-4">
        <el-checkbox v-model="autoSwitchUnpaid" :disabled="!policy.enabled">
          确认未扣款/失败后自动换卡（关闭=发 no_auto_card_switch）
        </el-checkbox>
        <el-checkbox v-model="policy.strict_card_preference" :disabled="!policy.enabled">严格使用已保存的卡头范围</el-checkbox>
      ...
      <p class="text-xs text-subtle mt-3">
        ...
        保存后同步 GPT 用卡优先级；实际可用卡以卡台调度结果为准。自动开卡由卡台 CDK 流程控制，现有接口未提供可生效的关闭开关。
      </p>
    </div>
    ...
    <div class="card">
      <div class="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div>
          ...
          <p class="text-sm text-muted mt-1">
            ...
            <strong>不同邮箱</strong>判为疑似卡问题（仅本站排除，下次 CDK 选卡跳过）；
      ...
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 mb-4">
        ...
        <div class="flex items-end pb-1">
          <span class="text-sm text-muted">排除仅作用于本站 CDK，主站卡片不会被自动冻结。</span>
        </div>
        <div class="flex items-end pb-1">
