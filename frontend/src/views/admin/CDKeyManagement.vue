...
import { beginIssue, finishIssue, pendingIssue } from '../../lib/issue-request'
...
async function issue() {
  ...
  try {
    ...
    const codes: string[] = issued.map(extractFullCode).filter(Boolean)
    if (!codes.length) {
    ...
    if (codes.length < purchaseCount) okMsg = `部分成功：申请 ${purchaseCount} 张，已生成 ${codes.length} 张；其余未生成。`
...
function downloadCodes() {
  ...
  const a = document.createElement('a')
  a.href = URL.createObjectURL(blob)
  a.download = `cdk-${recentMeta.value?.plan || form.plan || 'batch'}-${Date.now()}.txt`
  a.click()
  URL.revokeObjectURL(a.href)
