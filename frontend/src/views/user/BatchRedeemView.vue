    ...
    <div class="portal-view-inner">
      ...
      <div class="card space-y-4">
        ...
        <template v-if="phase === 'input'">
          <div>
            <label for="batch-codes" class="block text-sm font-medium text-ink mb-1.5">
              {{ t('batch.cdkListLabel', { max: BATCH_MAX_KEYS }) }}
            </label>
            <textarea
              id="batch-codes" v-model="cdkText"
              rows="6"
        ...
        <template v-else>
              ...
              >
                {{
                  autoSubmitting
                    ...
                    : pauseRequested ? '继续提交剩余任务' : t('batch.autoSubmit', {
                        n: sessionPool.length,
                      })
                }}
              </button>
              <button v-if="autoSubmitting" type="button" class="btn-secondary w-full" :disabled="pauseRequested" @click="pauseRequested = true">{{ pauseRequested ? '正在暂停后续任务…' : '暂停后续提交' }}</button>
              <p v-if="autoSubmitting || pauseRequested" class="text-xs text-muted">暂停只停止启动后续任务，已经开始处理的任务会继续。</p>
            </div>
          ...
          <button type="button" class="btn-secondary w-full" :disabled="autoSubmitting || submitting" @click="resetAll">
            {{ t('batch.reset') }}
          </button>
        </template>
      </div>
...
import { redemptionRequestId, previewErrorMessage } from '../../lib/redemption'
...
watch(
  ...
  async (id) => {
    ...
    const hasPool = sessionPool.value.length > 0
...
async function submitOne(
  item: BatchItem,
  credential: { mode: 'session'; session: string; imported?: ImportedSession },
): Promise<'ok' | 'fail'> {
...
async function runPreflightRedeem(
  item: BatchItem,
  credential: { mode: 'session'; session: string },
  ...
  try {
    if (!redemptionToken) {
      ...
      if (!preview.r.ok || (preview.data?.code != null && Number(preview.data.code) !== 0)) {
        const msg = previewErrorMessage(preview.data)
    ...
    const client_request_id = redemptionRequestId(item.clientRequestId)
    updateItem(item.id, { clientRequestId: client_request_id })
    submitted = true
    ...
    if (!redeem.r.ok && (redeem.r.status >= 500 || redeem.r.status === 409)) {
      updateItem(item.id, { status: 'processing', progressMsg: '提交结果正在确认，请勿重复提交', error: '' })
    ...
    if (submitted) {
      updateItem(item.id, { status: 'processing', progressMsg: '连接中断，正在确认原订单结果', error: '' })
      startPoll(item.id, redemptionToken)
      return 'ok'
    }
    updateItem(item.id, {
      status: 'failed',
      progressMsg: '校验连接中断，尚未提交兑换，请稍后重试',
      error: '网络异常',
    })
    return 'fail'
  }
...
onUnmounted(() => { pauseRequested.value = true; stopAllPolls() })
</script>
