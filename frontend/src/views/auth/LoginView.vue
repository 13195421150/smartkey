<template>
  <div class="portal-shell portal-auth relative min-h-screen flex items-center justify-center px-6 py-10">
    ...
    <div class="w-full max-w-md animate-slideInUp">
      <div class="card space-y-6 !p-7">
        <div class="text-center">
          <img src="/black-cat.png" alt="黑猫标志" class="auth-logo" width="84" height="84" />
        ...
        <div class="form-group">
          <label for="admin-user">{{ t('login.username') }}</label>
          <input id="admin-user" v-model="form.username" class="input" :placeholder="t('login.usernamePlaceholder')" autocomplete="username" @keyup.enter="submit" />
        </div>

        <div class="form-group">
          <label for="admin-password">{{ t('login.password') }}</label>
          <input id="admin-password" v-model="form.password" class="input" type="password" :placeholder="t('login.passwordPlaceholder')" autocomplete="current-password" @keyup.enter="submit" />
        </div>
