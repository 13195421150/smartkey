...
func TestSellablePlansRespectsPurchasableAndEmptyRegistry(t *testing.T) {
	no := false
	r := &PlansResponse{RegistryPresent: true, Plans: map[string]PlanInfo{"plus": {Enabled: true}}, Registry: []PlanRegistryItem{{Key: "plus", Product: "gpt", Purchasable: &no}}}
	if len(r.SellablePlans()) != 0 {
		t.Fatal("unavailable plan advertised")
	}
	r.Registry = nil
	if len(r.SellablePlans()) != 0 {
		t.Fatal("explicitly empty registry must not fall back to pricing table")
	}
}
