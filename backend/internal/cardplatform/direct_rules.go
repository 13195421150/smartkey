...
type DirectCardRule struct {
	...
	ExcludeArchivedCards *bool                  `json:"exclude_archived_cards,omitempty"`
}

// CardPoolPreview is read-only and does not create or fund an order.
type CardPoolCandidate struct {
	...
	LightUsed    int         `json:"light_used"`
	LightRemain  int         `json:"light_remain"`
	Pro20Used    int         `json:"pro20_used"`
	Pro20Remain  int         `json:"pro20_remain"`
	AvailableUSD json.Number `json:"available_usd"`
}
type CardPoolPreview struct {
	Rule       DirectCardRule      `json:"rule"`
	Candidates []CardPoolCandidate `json:"candidates"`
	Pick       *CardPoolCandidate  `json:"pick"`
}

func (c *Client) PreviewCardPool(ctx context.Context, plan string) (*CardPoolPreview, error) {
	q := url.Values{"product": {"gpt"}, "plan": {plan}, "card_mode": {"auto_existing"}}
	raw, err := c.doOpenAPI(ctx, http.MethodGet, "/gpt-direct/card-pool/schedule?"+q.Encode(), nil, "")
	if err != nil {
		return nil, err
	}
	var out CardPoolPreview
...
type CardUsage struct {
	...
	Light         CardUsageBucket `json:"light"`
	Pro20         CardUsageBucket `json:"pro20"`
	CountFailures bool            `json:"count_failures"`
...
func (c *Client) GetCardUsage(ctx context.Context, id int64) (*CardUsage, error) {
	raw, err := c.doOpenAPI(ctx, http.MethodGet, "/gpt-direct/cards/"+strconv.FormatInt(id, 10)+"/usage?product=gpt", nil, "")
	if err != nil {
		return nil, err
	}
	var out CardUsage
