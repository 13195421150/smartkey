...
func CardPlatformIssueCDKs(c *gin.Context) {
	...
	if cli := cardplatform.NewFromSettings(); cli != nil {
		...
		// Do not purchase a code unless its current availability was verified.
	}
	...
	if err != nil {
		// A list of historical unused codes cannot prove this purchase succeeded.
		// The browser retains the Idempotency-Key for a retry of this same purchase.
		writeCardErr(c, err)
		return
	}
	u, _ := c.Get("username")
	...
	for _, it := range res.List {
		...
		if !ok {
			if up := it.FullCodeText(); up != "" {
				full, ok = up, true
				canAlias = db.SaveCardplatformCDKCodeWithStatus(it.ID, up, it.CodePrefix, it.Plan, it.FeeAmountMinor, it.Status) == nil
			}
		}
		row := rowOut{
...
func PublicCDKPreview(c *gin.Context) {
	...
	code, ok := resolveCDKInput(c, str(body["code"]))
	if !ok {
		return
	}
	cli := cardplatform.NewFromSettings()
	...
	if publicResponseFailed(st, raw) && confirmedConsumedCDK(c.Request.Context(), code) {
...
func PublicCDKPreflight(c *gin.Context) {
	...
	code, _ := db.FindCodeByRedemptionToken(tok)
...
func PublicCDKRedeem(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "暂无法核对排除卡片，请稍后重试"})
		return
	}
	if len(ids) > 0 {
		seen := map[int64]bool{}
		...
		if supplied, ok := body["exclude_card_ids"].([]any); ok {
			for _, v := range supplied {
				if id := anyToInt64(v); id > 0 && !seen[id] {
					ids = append(ids, id)
					seen[id] = true
				}
			}
...
func PublicCDKResultByCode(c *gin.Context) {
	...
	displayCode := code
	code, ok := resolveCDKInput(c, code)
	if !ok {
		return
	}
