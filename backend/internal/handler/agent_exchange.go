...
func PublicAgentCDKExchange(c *gin.Context) {
	...
	code, resolved := resolveCDKInput(c, code)
	if !resolved {
		return
	}
	...
	storedNew := db.SaveCardplatformCDKCode(newIt.ID, newCode, newPrefix, plan, newIt.FeeAmountMinor) == nil
	...
	if storedNew {
		newCode = cdkcode.Display(newCode, plan)
		newPrefix = cdkcode.Display(newPrefix, plan)
	}
