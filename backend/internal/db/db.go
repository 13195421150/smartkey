...
func createTables() error {
	queries := []string{
		...
		// Latest public submission outcome; never store request credentials here.
		`CREATE TABLE IF NOT EXISTS cdk_attempt_diagnostics (
			cdk_code TEXT PRIMARY KEY,
			phase TEXT NOT NULL,
...
func SaveCardplatformCDKCodeWithStatus(upstreamID int64, code, prefix, plan string, feeMinor int64, status string) error {
	...
	if aliasPlan != "" {
		...
		code, plan = canonical, aliasPlan
		prefix, _ = cdkcode.Parse(prefix)
		// Browser cache backfill must not revive an already consumed/disabled code.
		if _, savedPlan, _, savedStatus, exists := LookupStoredCDKByCode(code); exists {
			if !strings.EqualFold(savedPlan, plan) {
				return ErrCDKAlias
			}
			status = savedStatus
		}
